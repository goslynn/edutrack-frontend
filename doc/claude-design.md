...
| `templates/content/{index.html,ui.jsx,explorer.jsx,app.jsx,data.js}` (Contenido — explorador Drive del árbol MS-Content) | `src/pages/dashboard/ContenidosPage.tsx` (contenedor) + `src/components/contenidos/*` (screen, explorer, árbol, diálogos de nodo/subida/descarga, meta) + `src/types/contenidos.ts` + `src/api/contenidos.ts` (BFF `/bff/contenidos`) + `src/hooks/useContenidos.ts`. El selector de rol del prototipo se descarta (RBAC real = enabled/disabled/hidden, sin switcher) |
